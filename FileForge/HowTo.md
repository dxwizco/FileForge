...
### Option 1 — Go into FileForge folder and run or Copy FileForge folder wherever you want

Example:

```powershell
# Change folder to the execution file location
CD PS D:\WIP-Learn\dx-test\FileForge>

# Run the command giving which file to use and where to create
.\Scaffold.ps1 -File react-app -Target "D:\WIP-Learn\dx-test"
```
...
- `&` → PowerShell call operator. Executes the script path safely.
- `Scaffold.ps1` → The scaffold engine script.
- `-File react-app` → Selects the scaffold definition file `files\react-app.txt`.
- `-Target "D:\WIP-Learn\dx-test"` → Defines the destination root where generated folders/files are created.

or

```powershell
& "D:\WIP-Learn\dx-test\FileForge\Scaffold.ps1" -File react-app -Target "D:\WIP-Learn\dx-test"
```
